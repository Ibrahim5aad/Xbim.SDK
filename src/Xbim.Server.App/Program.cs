...
using Xbim.Server.Abstractions.Auth;
using Xbim.Server.Abstractions.Processing;
using Xbim.Server.Abstractions.Storage;
using Xbim.Server.App.Auth;
using Xbim.Server.App.Endpoints;
using Xbim.Server.App.HealthChecks;
using Xbim.Server.App.Middleware;
using Xbim.Server.App.Processing;
using Xbim.Server.App.RateLimiting;
using Xbim.Server.Persistence.EfCore;
using Xbim.Server.Persistence.EfCore.Extensions;
using Xbim.Server.Processing;
using Xbim.Server.Storage.AzureBlob;
using Xbim.Server.Storage.LocalDisk;
using Xbim.Server.App.Swagger;
...
builder.Services.Configure<Xbim.Server.App.Auth.OAuthTokenOptions>(
    builder.Configuration.GetSection("OAuth"));
builder.Services.AddSingleton<Xbim.Server.App.Auth.IOAuthTokenService, Xbim.Server.App.Auth.OAuthTokenService>();
...
app.UseExceptionHandler(exceptionHandlerApp =>
{
    exceptionHandlerApp.Run(async context =>
    {
        ...
        if (exception is Xbim.Server.App.Auth.InsufficientScopeException scopeException)
        {
        ...
        else if (exception is Xbim.Server.App.Auth.WorkspaceIsolationException isolationException)
        {
            context.Response.StatusCode = StatusCodes.Status403Forbidden;
            context.Response.ContentType = "application/json";
            await context.Response.WriteAsJsonAsync(new
        ...
        else if (exception is Xbim.Server.App.Auth.ForbiddenAccessException)
        {
            context.Response.StatusCode = StatusCodes.Status403Forbidden;
            context.Response.ContentType = "application/json";
            await context.Response.WriteAsJsonAsync(new
